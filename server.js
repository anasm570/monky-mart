...
class GameWorld {
    constructor() {
        ...
        this.money = 1000;             // النقود المشتركة
        this.unlockedCrops = ['corn'];
    }
    getClientData() {
        return {
            money: this.money,
...
function authenticate(username, password) {
    const players = loadPlayers();
    if (players[username] && players[username].password === password) return true;
    if (!players[username]) {
...
// جعل الصفحة الرئيسية تخدم index+.html مباشرة
app.get('/', (req, res) => {
    const htmlFile = path.join(__dirname, 'index+.html');
    if (fs.existsSync(htmlFile)) {
        res.sendFile(htmlFile);
    } else {
        res.status(404).send('❌ الملف index+.html غير موجود في جذر المشروع');
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('login', ({ username, password }) => {
        if (authenticate(username, password)) {
            ...
            // دمج التقدم المحفوظ مع العالم الحالي
...
server.listen(PORT, () => {
    console.log(`✅ Monkey Mart متعدد اللاعبين يعمل على http://localhost:${PORT}`);
    console.log(`📄 الملف الرئيسي: index+.html (موجود في ${__dirname})`);
});